...
variable "spn_authority" {
  ...
  default     = "https://login.partner.microsoftonline.cn"
}
...
resource "azurerm_virtual_machine_extension" "custom_script" {
  ...
  settings = <<SETTINGS
    {
      "fileUris": [
          "https://eslzfiles.blob.core.chinacloudapi.cn/azure-arc/Bootstrap-mooncake.ps1"
      ],
      "commandToExecute": "powershell.exe -ExecutionPolicy Bypass -File Bootstrap-mooncake.ps1 -adminUsername ${var.admin_username} -spnClientId ${var.spn_client_id} -spnClientSecret ${var.spn_client_secret} -spnTenantId ${var.spn_tenant_id} -spnAuthority ${var.spn_authority} -subscriptionId ${data.azurerm_subscription.primary.subscription_id} -resourceGroup ${data.azurerm_resource_group.rg.name} -acceptEula ${var.accept_eula} -registryUsername ${var.registry_username} -registryPassword ${var.registry_password} -azureLocation ${data.azurerm_resource_group.rg.location}  -stagingStorageAccountName ${var.storage_account_name} -workspaceName ${var.workspace_name} -templateBaseUrl ${var.template_base_url} -flavor ${var.deployment_flavor} -automationTriggerAtLogon ${var.trigger_at_logon}  -githubUser ${var.github_username}"
    }
SETTINGS
}
